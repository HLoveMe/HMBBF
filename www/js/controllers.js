...
conCtrl.controller("mainCtrl", function($cordovaDevice, $scope, $rootScope, $http, $timeout, $ionicSideMenuDelegate, $state, transferServer, $shareSDK, $ionicTabsDelegate, $ionicSideMenuDelegate, $UserDefaluts, dataService, $cordovaNetwork, $ionicPopup, $ionicNavBarDelegate, $ionicPlatform) {
        //APP注册
        //这是使用默认
        $rootScope.user_id = 1760
    ...
    .controller('homeCtrl', function($scope, dataService, $ionicSlideBoxDelegate, $state, $ImageBox, $timeout, $DataManager) {
        //轮播图
        $scope.adArr = []
            //消息滚动
        $scope.newsArr = []
        ...
        ionic.Platform.ready(function() {
            if (typeof DataManager != "undefined") {
                $DataManager.operation({
                    table: "ADImageList"
                }, ConnectArgs.Operation.select).then(function(data) {
                    $scope.adArr = data
        ...
        dataService.getData("api.php/index/index", {
                ...
                $timeout(function() {
                    $DataManager.operation({
                        table: "ADImageList"
                    }, ConnectArgs.Operation.delete).then(function() {
                        $DataManager.operation({
                            table: "ADImageList",
                            data: data["ad"]
                        }, ConnectArgs.Operation.save)
                    })
        ...
        $scope.moduleClick = function(index) {
            if (index == 0) {
                ...
                $state.go("tab.homeFourModule", { cache: false })
            }
        }
    })
    .controller("meetCtrl", function($rootScope, $scope, $http, $state, dataService, $ionicScrollDelegate) {
        ...
        dataService.getData("api.php/guide/venue_info").then(function(data){
            console.log(data);
            $scope.venue = data
            $scope.titles[1].hasData = true
        }, function(){
            alert("load error")
        })
        //表头点击
        ...
        //场馆介绍图片介绍
        $scope.venue_img_Click = function(model){
...
.controller("newsCtrl", function($rootScope, $scope, $http, $state, dataService, $ionicScrollDelegate, $timeout, $shareSDK, transferServer, $DataManager) {
        $scope.showInfinite = false
        $scope.page = 1
        $scope.newsArr = []
            //读取本地数据
        if (window & typeof ConnectArgs != "undefined") {
            $DataManager.operation({
                table: "news"
            }, ConnectArgs.Operation.select).then(function(argument) {
                $scope.newsArr = $scope.newsArr.concat(argument)
            })
        }
        ...
        $scope.loadData = function(more) {
                ...
                dataService.getData("api.php/information/index", {
                    ...
                    if (window & typeof ConnectArgs != "undefined") {
                        $DataManager.operation({ table: "news" }, ConnectArgs.Operation.delete).then(function() {
                            $DataManager.operation(ops, ConnectArgs.Operation.save)
                        })
                    }
                    if ($scope.page == 1) { $scope.newsArr.length = 0 }
